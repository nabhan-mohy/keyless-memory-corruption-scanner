...
that Phase 4 consumes, and it exposes the two classification helpers that
:mod:`kmcs.analysis.classifier` consumes.
"""
...
__all__ = [
    ...
    "CrashParser",
    "parse_crash",
    "ASAN_ERROR_TO_CLASSIFICATION",
    "UBSAN_MESSAGE_TO_CLASSIFICATION",
...
# ---------------------------------------------------------------------- patterns
# --- ASan header (multi-word safe) -------------------------------------------

# The prefix is matched with ``.match()`` so only headers at the start of a
# line are recognised.  The error type after the colon is captured separately
# so that multi-word ASan error names
# ("attempting free on address which was not malloc()-ed") survive intact.
_ASAN_HEADER_PREFIX = re.compile(r"==(?P<pid>\d+)==ERROR:\s*AddressSanitizer:\s*")

# Boundaries that terminate the error-type text in an ASan header line.
_ASAN_ERROR_BOUNDARIES: tuple[re.Pattern[str], ...] = (
    re.compile(r"\s+on\s+(?:\S+\s+)?address\s+0x"),
    re.compile(r"\s+at\s+pc\s+0x"),
    re.compile(r"\s+in\s+thread\s+"),
    re.compile(r":\s+0x"),
)

def _extract_asan_error_and_address(rest: str) -> tuple[str | None, int | None]:
    """Return ``(error_type, faulting_address)`` from the text after the prefix.

    ASan error types are not always single tokens.  ``heap-buffer-overflow``
    and ``use-after-free`` are, but ``attempting free on address which was
    not malloc()-ed`` is not.  We locate the earliest boundary marker that
    introduces address or thread information, and take everything before it.
    """
    earliest = len(rest)
    for pattern in _ASAN_ERROR_BOUNDARIES:
        match = pattern.search(rest)
        if match is not None and match.start() < earliest:
            earliest = match.start()

    error_type = rest[:earliest].strip().rstrip(":") or None

    address: int | None = None
    address_match = re.search(r"0x[0-9a-fA-F]+", rest)
...
_ASAN_FREE_STACK_HEADER = re.compile(
    r"freed by thread\s+T?(?P<tid>\d+)?\s+here:"
)
_ASAN_ALLOC_STACK_HEADER = re.compile(
    r"allocated by thread\s+T?(?P<tid>\d+)?\s+here:"
)
...
# UBSan emits two forms.  The first has a ``file:line:col:`` prefix; the
# second is a bare ``runtime error: ...`` line with no location.  Both are
# parsed.
...
_UBSAN_BARE_RUNTIME_ERROR = re.compile(
    r"^runtime error:\s+(?P<message>.+)$"
)
...
# Exposed publicly so :mod:`kmcs.analysis.classifier` can reuse them.

ASAN_ERROR_TO_CLASSIFICATION: dict[str, CrashClassification] = {
    ...
    "use-after-poison": CrashClassification.USE_AFTER_FREE,
    "double-free": CrashClassification.DOUBLE_FREE,
    "attempting double-free": CrashClassification.DOUBLE_FREE,
    "attempting free on address which was not malloc()-ed": CrashClassification.INVALID_FREE,
    "attempting free on address which was not malloced": CrashClassification.INVALID_FREE,
    "attempting-free-on-address-which-was-not-malloc-ed": CrashClassification.INVALID_FREE,
    "new-delete-type-mismatch": CrashClassification.INVALID_FREE,
    "container-overflow": CrashClassification.HEAP_BUFFER_OVERFLOW,
...
# Backwards-compatibility alias for code that imported the private name.
_ASAN_ERROR_TO_CLASSIFICATION = ASAN_ERROR_TO_CLASSIFICATION


UBSAN_MESSAGE_TO_CLASSIFICATION: tuple[
    tuple[re.Pattern[str], CrashClassification], ...
] = (
...
_UBSAN_MESSAGE_TO_CLASSIFICATION = UBSAN_MESSAGE_TO_CLASSIFICATION


# --- Public classification helpers ------------------------------------------


def classify_asan_error_type(error_type: str | None) -> CrashClassification | None:
    """Map an ASan error type to a :class:`CrashClassification`, or ``None``.

    ``None`` means the error type is not one KMCS recognises.  That is
    deliberate: the classifier falls back to ``UNKNOWN`` rather than guessing.
    """
    if error_type is None:
        return None
    return ASAN_ERROR_TO_CLASSIFICATION.get(error_type)


def classify_ubsan_message(message: str | None) -> CrashClassification | None:
    """Map a UBSan message to a :class:`CrashClassification`, or ``None``."""
    if message is None:
        return None
    for pattern, classification in UBSAN_MESSAGE_TO_CLASSIFICATION:
...
class CrashParser:
    ...
    def __init__(self) -> None:
        # Per-instance collection target during ASan parsing.  Keyed on
        # ``id(report)`` so that concurrent parses never collide.
        self._current_collection: dict[int, str] = {}
    ...
    def _parse_sanitizer_reports(
        ...
        # When the caller knows the target was built with exactly one
        # sanitizer, a report whose kind could not be inferred from the text
        # is labelled with that sanitizer.  We never guess between two.
    ...
    def _parse_asan(self, lines: list[str], report: CrashReport) -> None:
        ...
        total = len(lines)
        while i < total:
            prefix_match = _ASAN_HEADER_PREFIX.match(lines[i])
            if prefix_match is None:
                i += 1
                continue

            rest = lines[i][prefix_match.end():]
            error_type, faulting_address = _extract_asan_error_and_address(rest)
    ...
    def _find_block_end(lines: list[str], start: int) -> int:
        """A block ends when a line starts a new report."""
        i = start + 1
        total = len(lines)
        while i < total:
            line = lines[i]
            if _ASAN_HEADER_PREFIX.match(line):
                return i
            if _MSAN_USE.search(line) or _TSAN_RACE.search(line):
                return i
            if (
                _UBSAN_RUNTIME_ERROR.match(line)
                or _UBSAN_BARE_RUNTIME_ERROR.match(line)
            ):
                return i
            i += 1
        return total
    ...
    def _parse_ubsan(self, lines: list[str], report: CrashReport) -> None:
        for i, line in enumerate(lines):
            stripped = line.strip()
            match = _UBSAN_RUNTIME_ERROR.match(stripped)
            if match is None:
                match = _UBSAN_BARE_RUNTIME_ERROR.match(stripped)
            if match is None:
                continue
    ...
    def _populate_sanitizer_classification(self, report: CrashReport) -> None:
        ...
        if sr.sanitizer is SanitizerKind.UNDEFINED and sr.error_type:
            mapped = classify_ubsan_message(sr.error_type)
            if mapped is not None:
                report.classification = mapped
                return
